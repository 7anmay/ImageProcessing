...
def marker_loc():
    cent_blue = np.zeros((1, 2), np.int64)
    cent_pink = np.zeros((1, 2), np.int64)
...
def angle(cent_1,cent_2,goal1):
    if (cent_1[0][0] == cent_2[0][0]):
        m2 = (((cent_1[0][1] + cent_2[0][1]) / float(2)) - goal1[1]) / float(((cent_1[0][0] + cent_2[0][0]) / float(2)) - goal1[0])
        angle = math.atan(m2)
    elif(cent[0][0] == goal1[0]):
        cent[0][0]=cent[0][0] + 1
        m1 = (cent_1[0][1] - cent_2[0][1]) / float(cent_1[0][0] - cent_2[0][0])
        m2 = (((cent_1[0][1] + cent_2[0][1]) / float(2)) - goal1[0][1]) / float(((cent_1[0][0] + cent_2[0][0]) / float(2)) - goal1[0])
        # angle is positive towards first variable in the formula for angle
        angle = (math.atan((m2 - m1) / float(1 + (m1 * m2)))) * 180 / float(np.pi)
    else:
        m1 = (cent_1[0][1] - cent_2[0][1]) / float(cent_1[0][0] - cent_2[0][0])
        m2 = (((cent_1[0][1] + cent_2[0][1]) / float(2)) - goal1[1]) / float(((cent_1[0][0] + cent_2[0][0]) / float(2)) - goal1[0])
        # angle is positive towards first variable in the formula for angle
...
def direction(angle,cent_1,cent_2,goal1):
    dis2 = (math.sqrt(math.pow(int(goal1[1] - cent_2[0][1]), 2) + math.pow(int(goal1[0] - cent_2[0][0]), 2)))
    dis1 = (math.sqrt(math.pow(int(goal1[1] - cent_1[0][1]), 2) + math.pow(int(goal1[0] - cent_1[0][0]), 2)))
    m1 = (cent_1[0][1] - cent_2[0][1]) / float(cent_1[0][0] - cent_2[0][0])
    d = m1*cent_1[0][0]-cent_1[0][1]
    if (cent_1[0][1] == cent_2[0][1]):
        if (dis1<dis2):
            ...
            if(((cent_2[0][1]-cent_1[0][1])*(cent_1[0][0]-goal1[0]))>0):
                 dir = 'anti_clockwise'
                 angle = 90
            else:
                dir = 'clockwise'
        ...
        if (dis1<=dis2):
            if((d*(goal1[1]-(m1*goal1[0])+d))>0):
                dir = 'anti_clockwise'
                angle = math.fabs(float(angle))
            else:
                dir = 'clockwise'
                angle = math.fabs(float(angle))
        else:
            if ((d * (goal1[1] - (m1 * goal1[0]) + d)) > 0):
                dir ='anti_clockwise'
                angle = 180 - math.fabs(float(angle))
            else:
                dir = 'clockwise'
                angle = 180 - math.fabs(float(angle))
    return dir, angle

def distance(cent,goal1):
    #finding euclidean distance
    dis = math.sqrt(math.pow((cent[0][0]-goal1[0]),2) + math.pow((cent[0][1]-goal1[1]),2))
    return dis
