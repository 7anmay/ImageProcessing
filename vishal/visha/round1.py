...
cent =np.zeros((1,2))
def centroid(cent_pink,cent_2):
    x =int(cent_1[0][0]+cent_blue[0][0])/2
    y = int(cent_pink[0][1]+cent_blue[0][1])/2
    cent[0][0]=x
    cent[0][1]=y
...
def distance(centr,goal):
    ...
    dis = math.sqrt(math.pow((centr[0][0]-goal[0][0]),2) + math.pow((centr[0][1]-goal[0][1]),2))
    return dis


def angle(cent_pink,cent_blue,goal1):

    if ((cent_pink[0][0] - cent_blue[0][0]) == 0 and not(int((cent_pink[0][0] + cent_blue[0][0]) / float(2)) - goal1[0][0]) == 0):
        m2 = (((cent_pink[0][1] + cent_blue[0][1]) / float(2)) - goal1[0][1]) / float(((cent_pink[0][0] + cent_blue[0][0]) / float(2)) - goal1[0][0])
    ...
    elif(int(((cent_pink[0][0] + cent_blue[0][0]) / float(2)) - goal1[0][0]) == 0 and not((cent_pink[0][0] - cent_blue[0][0]) == 0)):
        m1 = (cent_pink[0][1] - cent_blue[0][1]) / float(cent_pink[0][0] - cent_blue[0][0])
    ...
    elif(int(cent_pink[0][0] - cent_blue[0][0]) == 0 and int(((cent_pink[0][0] + cent_blue[0][0]) / float(2)) - goal1[0][0]) == 0):
        if (dis1<dis2):
            angle = 1
        else:
            angle = 180
...
def direction(angle):
    ...
    if ((cent_pink[0][0] - cent_blue[0][0]) == 0 and (int((cent_pink[0][0] + cent_blue[0][0]) / float(2)) - goal1[0][0]) == 0):
        if angle<0:
            dir = 'anticlockwise'
        else:
            dir = 'clockwise'
...
########################################################################################
dis1 = distance(cent_1,goal)
dis2 = distance(cent_2,goal)
dis = distance(cent,goal)
centroid(cent_1,cent_2)
ang = angle(cent_1,cent_2,goal)
dir = direction(ang)
cv2.circle(img,(goal[0][0],goal[0][1]),3,(255,255,87),-1)
cv2.line(img,(cent_1[0][0],cent_1[0][1]),(cent_2[0][0],cent_2[0][1]),(0,255,255),2)
cv2.line(img,(cent[0][0],cent[0][1]),(goal[0][0],goal[0][1]),(0,0,255),2)
...
for i in goal:
    while(True):
        ...
        ang = angle(cent_blue, cent_pink, i)
        dir, ang = direction(ang, cent_blue, cent_pink, i )
        print ang
        if (dir == 'clockwise'):
            dir = 'o'
        else:
        ...
        while(ang<100):
            ...
            dir, ang = direction(ang, cent_pink, cent_blue, i)
