...
def marker_loc():
    ...
    while True:
        ...
        lower_blue = np.array([73, 83, 160])
        upper_blue = np.array([120, 240, 255])

        lower_pink = np.array([122, 86, 98])
        upper_pink = np.array([180, 186, 255])
...
cv2.imwrite('Fortress-1.png',arena)
...
ret,thresh_arena = cv2.threshold(arena_gray,150,255,cv2.THRESH_BINARY)
...
for cnt_a in contours_arena:
    ...
    if(area>1500):
        corners_1[i][0] = cnt_a[0][0][0]
        corners_1[i][1] = cnt_a[0][0][1]
        corners_2[i][0] = cnt_a[1][0][0]
        corners_2[i][1] = cnt_a[1][0][1]
        approx_contours.append(cnt_a)
        y = cv2.moments(cnt_a)
        moments.append(y)
        i = i + 1
