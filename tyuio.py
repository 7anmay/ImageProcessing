...
def marker_loc():
    ...
    while True:
        ...
        lower_blue = np.array([105, 140, 76])
        upper_blue = np.array([111, 255, 255])

        lower_pink = np.array([116, 50, 101])
        upper_pink = np.array([185, 121, 170])
...
col = [(255,255,1),(0,0,254),(1,255,1),(254,0,0),(248,8,245)]
...
ret,thresh = cv2.threshold(imgray_invert ,70 , 255 , cv2.THRESH_BINARY)
...
for i in range(len(e_roi)):
    ...
    ret, thresh_roi = cv2.threshold(img_roi_invert, 50, 255, cv2.THRESH_BINARY)
...
ret,thresh_arena = cv2.threshold(arena_gray,130,233,cv2.THRESH_BINARY)
...
goal = np.zeros(((len(final_sequence)+ 1),2),np.uint32)
...
for guide in path:
    while True:
        ...
        while (math.fabs(ang) < 30):
            ...
            cv2.line(fortress, (cent_pink[0][0], cent_pink[0][1]), (cent_blue[0][0], cent_blue[0][1]), (255, 255, 255),
                     thickness=3)
            cv2.line(fortress, (point[0], point[1]), (cent[0][0], cent[0][1]), (255, 255, 255), thickness=3)
            print 'distance= ', dist
            cv2.imshow('path', fortress)
