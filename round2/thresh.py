...
ret,thresh = cv2.threshold(imgray_invert ,65 , 255 , cv2.THRESH_BINARY)
...
for i in range(len(e_roi)):
    ...
    ret, thresh_roi = cv2.threshold(img_roi_invert, 65, 255, cv2.THRESH_BINARY)
...
#arena = cv2.imread('round1arena.jpg')
...
ret,thresh_arena = cv2.threshold(arena_gray,144,255,cv2.THRESH_BINARY)
...
goal = np.zeros((len(final_sequence),02),np.uint32)
